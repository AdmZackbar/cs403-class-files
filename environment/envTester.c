...
static void printLocalEnv(LEXEME *env);
...
int main()
{
    ...
    printLocalEnv(env);
    changeValue(env, "y", newLEXEMEdouble(3.0, -1));
    printEnv(env);
}
...
static LEXEME *changeValue(LEXEME *env, char *name, LEXEME *newVal)
{
    ...
    printf("\n");
    LEXEME *oldVal = setValueEnv(env, newLEXEMEstring(ID, name, -1), newVal);
